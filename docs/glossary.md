...
| **`/api/*` proxy hop pattern** | **เพิ่ม session 30, Jul 23 (ADR-024)** — กฎที่ยืนยันซ้ำจากการตรวจ data-flow graph ทั้ง platform: browser (client component, `"use client"`) **ห้ามยิง URL ภายนอกตรง** ไม่ว่าจะ Hub v1 (Railway) หรือ FB Backend (Railway อีกตัว) เสมอต้องผ่าน Next.js `app/api/.../route.ts` ก่อน ให้ secret (`HUB_SECRET`/`x-hub-token`) อยู่ฝั่ง server เท่านั้น — มี 2 รูปแบบที่ต้องแยกให้ออก: (1) **thin proxy → Hub v1/FB Backend** (เช่น `/api/blog/*`, `/api/property/*`, `/api/fb/publish`) ไม่แตะ Supabase เอง แค่ forward request พร้อม token (2) **service_role ตรงเข้า Supabase** (เช่น `/api/leads`, `/api/projects`, `/api/deals`, `/api/qc/accuracy`) ไม่ผ่าน Hub เลย — ต้องเช็คโค้ดจริงเสมอว่าเป็นแบบไหนก่อนอธิบาย/วาดใน diagram ห้ามเดาจากชื่อ route |
| **Auth-first severity check** | **เพิ่ม session 30, Jul 23 (ISSUE-017)** — เวลาเจอ pattern "browser เรียก external URL ตรง" ห้ามสรุป severity ("security exposure") ทันทีจากแค่เห็น URL — ต้องเช็คก่อนว่า endpoint ปลายทางมี auth guard จริงไหม (เช่น Hub v1's global `x-hub-token` middleware ใน `server.cjs`) ถ้ามีและ request ที่เจอไม่มี token แนบ แปลว่าโอกาสสูงเป็น **"ฟีเจอร์พังเงียบๆ" (functional bug)** ไม่ใช่ "รูรั่วความปลอดภัย" (security incident) — คนละ severity คนละคำแนะนำแก้ ต้องอ่าน error-handling code จริงด้วยว่า request ที่ fail ถูกจัดการยังไง (silent swallow แล้วโชว์ mock ค้าง vs โชว์ error ตรงๆ) ก่อนสรุป |
