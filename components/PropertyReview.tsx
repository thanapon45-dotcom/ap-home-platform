...
export default function PropertyReview() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0d", color: "#eee", fontFamily: "'Prompt', sans-serif", padding: "32px 24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#888" }}>
            ทรัพย์ฝากขายจาก LINE — รอ Approve ก่อนขึ้นเว็บ
          </p>
        </div>
        <button
          onClick={fetchPending}
          style={{ background: "#1a1a1a", border: "1px solid #333", color: "#aaa", padding: "8px 16px", borderRadius: 6, cursor: "pointer", fontSize: 13 }}
        >
          🔄 Refresh
        </button>
      </div>
      ...
      {/* States */}
      {loading && (
        <div style={{ textAlign: "center", padding: 60, color: "#666" }}>กำลังโหลด...</div>
      )}
      {error && (
        <div style={{ background: "#2a1a1a", border: "1px solid #8b2222", borderRadius: 8, padding: 16, color: "#f87171", marginBottom: 20 }}>
          ❌ {error}
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        {visible.map(p => {
          ...
          return (
            <div key={p.id} style={{
              background: "#141414",
              border: `1px solid ${isPub ? "#1a4a1a" : "#222"}`,
              borderRadius: 10,
              padding: 24,
              position: "relative",
            }}>
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
                <span style={{ fontSize: 11, color: "#555", background: "#1a1a1a", padding: "3px 8px", borderRadius: 4 }}>
                  📱 LINE: {p.line_user_id?.slice(0, 10)}...
                </span>
                <span style={{ fontSize: 11, color: "#555", background: "#1a1a1a", padding: "3px 8px", borderRadius: 4 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12, marginBottom: 16 }}>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  <input value={edit.title} onChange={e => updateField(p.id, "title", e.target.value)}
                    style={inputStyle} disabled={isPub} />
                </div>

                <div>
                  <label style={labelStyle}>ประเภท</label>
                  <select value={edit.property_type} onChange={e => updateField(p.id, "property_type", e.target.value)}
                    style={inputStyle} disabled={isPub}>
                ...
                <div>
                  ...
                  <input type="number" value={edit.asking_price} onChange={e => updateField(p.id, "asking_price", e.target.value)}
                    style={inputStyle} placeholder="3500000" disabled={isPub} />
                </div>

                <div>
                  <label style={labelStyle}>ทำเล / ย่าน</label>
                  <input value={edit.location} onChange={e => updateField(p.id, "location", e.target.value)}
                    style={inputStyle} placeholder="ลำลูกกา" disabled={isPub} />
                </div>

                <div>
                  <label style={labelStyle}>โซน</label>
                  <input value={edit.zone} onChange={e => updateField(p.id, "zone", e.target.value)}
                    style={inputStyle} placeholder="รังสิต" disabled={isPub} />
                </div>

                <div>
                  <label style={labelStyle}>ห้องนอน</label>
                  <input type="number" value={edit.bedrooms} onChange={e => updateField(p.id, "bedrooms", e.target.value)}
                    style={inputStyle} min={0} max={20} disabled={isPub} />
                </div>

                <div>
                  <label style={labelStyle}>ห้องน้ำ</label>
                  <input type="number" value={edit.bathrooms} onChange={e => updateField(p.id, "bathrooms", e.target.value)}
                    style={inputStyle} min={0} max={20} disabled={isPub} />
                </div>

                <div>
                  <label style={labelStyle}>พื้นที่ใช้สอย (ตร.ม.)</label>
                  <input type="number" value={edit.area_sqm} onChange={e => updateField(p.id, "area_sqm", e.target.value)}
                    style={inputStyle} disabled={isPub} />
                </div>

                <div>
                  <label style={labelStyle}>ที่ดิน (ตร.ม.)</label>
                  <input type="number" value={edit.land_sqm} onChange={e => updateField(p.id, "land_sqm", e.target.value)}
                    style={inputStyle} disabled={isPub} />
                </div>

                <div style={{ gridColumn: "1 / -1" }}>
                  <label style={labelStyle}>หมายเหตุ / ข้อความต้นฉบับจาก LINE</label>
                  <textarea value={edit.notes} onChange={e => updateField(p.id, "notes", e.target.value)}
                    rows={3} style={{ ...inputStyle, resize: "vertical" }} disabled={isPub} />
                </div>
              </div>
              ...
              <div style={{ fontSize: 13, color: "#C9A84C", marginBottom: 16 }}>
                ...
                {edit.bedrooms ? ` · 🛏️ ${edit.bedrooms}` : ""}
                {edit.bathrooms ? ` · 🚿 ${edit.bathrooms}` : ""}
              ...
              {!isPub ? (
                <div style={{ display: "flex", gap: 12 }}>
                  <button
                    ...
                    style={{
                      background: isPublishing ? "#333" : "#C9A84C",
                      color: isPublishing ? "#666" : "#0d0d0d",
