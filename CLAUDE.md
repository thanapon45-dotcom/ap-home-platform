...
**สถานะ ณ ตอนปิด session**: โค้ดแก้แล้วในเครื่อง **ยังไม่ push** — ดู Pending Tasks ด้านล่าง
...
### 🔴 ต้องทำก่อนอื่นเมื่อเริ่ม session ถัดไป (session 30, Jul 23)
- [ ] **Push `components/DashboardOS.tsx`** — แก้ `useLiveData()` แล้วในเครื่อง (ดู Known Bugs #14) แต่ยังไม่ได้ push ขึ้น GitHub/Vercel เลย ณ ตอนปิด session นี้ — รัน PowerShell จาก `D:\ARCHI\01_PROJECTS\CORE\ap-home-platform`: `git add components/DashboardOS.tsx && git commit -m "Fix: OS Dashboard live-state proxies via /api/blog/state" && git push` (แยกบรรทัดถ้า PowerShell รุ่นเก่าไม่รู้จัก `&&`) — ก่อนทำงานอื่นต่อ ให้เช็คก่อนว่า push แล้วหรือยัง (`git log -1 -- components/DashboardOS.tsx` เทียบ `git log origin/main -1 -- components/DashboardOS.tsx`)
- [ ] หลัง deploy สำเร็จ verify ผ่าน Vercel `list_deployments` (เทียบ commit) + เปิด `/dashboard` จริงดูว่าขึ้น "🟢 LIVE" แทน "🔴 OFFLINE"
- [ ] อัปเดตกราฟ `memory/finnhouses-graph-view-v2.html` — ลบเส้นประแดง "insecure" (`OS Dashboard→Hub v1`) ออก เพราะ path จริงเปลี่ยนไปใช้ hop `/api/blog/*` แล้วหลัง fix นี้ deploy สำเร็จ
