...
Product Owner ขอแก้ ADR ที่ต้นเหตุ: อนุญาตบทความให้ความรู้เรื่องสร้างบ้าน แต่ห้ามอ้างว่า Finnhouses รับจ้างสร้างบ้านให้ลูกค้า — ดู [ADR-016 amendment](docs/v2/04-decisions/accepted/ADR-016.md) ซึ่ง sync กับ ADR-010 และ Business Model แล้ว
...
### 14. OS Dashboard `useLiveData()` ยิง Hub v1 ตรงจาก browser ไม่ผ่าน proxy — RESOLVED (session 30, Jul 23)
**ปัญหา**: หน้า `/dashboard` (Overview) โชว์ "🔴 OFFLINE — แสดงข้อมูล MOCK" ตลอดเวลา + การ์ด FB/Blog Engine โชว์ตัวเลขเดิมซ้ำๆไม่เคยขยับ (queue 8/12, published 2/3 ฯลฯ) — พบระหว่างตรวจ data-flow graph ให้ตรงกับโค้ดจริง (ADR-024) ไม่ใช่ Archi รายงานเข้ามาก่อน
**Root cause**: `components/DashboardOS.tsx`'s `useLiveData()` เขียนไว้ `fetch(\`${NEXT_PUBLIC_HUB_URL}/api/state\`)` ยิงตรงจาก browser เข้า Hub v1 ไม่มี header `x-hub-token` เลย ขัดกับกฎ security ด้านบน ("ห้าม call Railway URL โดยตรงจาก client-side") — ตรวจ `server.cjs` (บรรทัด 578-593) ยืนยันว่า Hub v1 มี global auth middleware ครอบทุก route ยกเว้น `/health*` เลยโดน `401` ทุกครั้งไม่มีข้อยกเว้น ตั้งแต่วันแรกที่เขียนโค้ดนี้ — error ถูก catch เงียบๆ ไม่เคยเรียก `setData()` ใหม่เมื่อ fail เลย `data` เลยค้างที่ `MOCK` ตลอดไป
**สำคัญ — ไม่ใช่ data leak จริง**: 401 บล็อกได้จริง ไม่มีข้อมูล Hub จริงหลุดออกมาทาง path นี้เลยสักครั้ง สิ่งที่ "เปิดเผย" มีแค่ hostname Railway (ก็โผล่เป็น plain text อยู่แล้วในหน้า Blog Runner) — ผลกระทบจริงคือ **functional bug** (dashboard โชว์ข้อมูลปลอมค้างตลอด) ไม่ใช่ security incident — เจอเพราะ Archi ขอ "ประเมินก่อน" แทนที่จะให้แก้ทันทีตอนแรกที่ flag เป็น "security deviation"
**แก้**: เปลี่ยน `useLiveData()` ให้เรียก `/api/blog/state` (proxy ที่มีอยู่แล้ว ใช้จริงโดย `Marketing.tsx`, ใส่ `x-hub-token` ฝั่ง server ถูกต้อง) แทนยิง Railway ตรง + เพิ่มเช็ค `json?.error` เพราะ route นี้ตอบ `200 + {error}` เวลา Hub fail แทน non-2xx status
**กฎใหม่**: เจอ pattern "browser ยิง external URL ตรง" ห้ามสรุป severity ทันทีจากแค่เห็น URL — ต้องเช็คก่อนว่า endpoint ปลายทางมี auth guard จริงไหม ถ้ามีและ request ไม่มี token แปลว่าโอกาสสูงเป็น "ฟีเจอร์พังเงียบๆ" ไม่ใช่ "รูรั่วความปลอดภัย" ต้องอ่าน error-handling code จริงก่อนสรุป severity เสมอ
**สถานะ ณ ตอนปิด session**: ✅ push แล้ว (commit `85e964d`) + verify deploy ผ่าน Vercel `list_deployments` ตรง commit จริง (`dpl_EvzvQdvpDATq14jWsrHKajLeipGJ`, `READY`) + อัปเดตกราฟลบเส้นประแดงออกแล้ว — ปิดครบ
**ดู**: `docs/issues-log.md` ISSUE-017, `docs/decisions.md` ADR-024

### 15. Overview vs Quality Gate tab ไม่ตรงกัน — RESOLVED (session 31, Jul 25)
**ปัญหา**: Archi สังเกตว่าหน้า Overview แสดง blog error จริง (`Blocked by Publish Guard: brand_guardrail_violation`) แต่หน้า Quality Gate tab โชว์ "ตัดสินใจทั้งหมด 0" ตลอด — ดูเหมือนขัดแย้งกัน
**Root cause**: ไม่ใช่บั๊กที่ dashboard เลย — ทั้งสองหน้าถูกต้องตามข้อมูลจริง แค่วัดคนละแหล่ง: Overview อ่านจาก `hub_state` (Hub v1, มีข้อมูลจริงเสมอเพราะ Gate ทำงานอยู่แล้วจาก workflow เดิม `10_ai_quality_gate`) ส่วน Quality Gate tab อ่านจาก Supabase `quality_gate_log` ซึ่งมี 0 แถวจริง เพราะ node "Log Quality Gate Decision" (ADR-021) อยู่ใน workflow `(11_gate_log)` เท่านั้น ซึ่งตอนนั้นยังไม่เคยถูก import/activate ใน n8n เลย (ของค้างจาก ADR-021)
**พบเพิ่มระหว่างแก้**: node "Log Quality Gate Decision" ใน `(11_gate_log)` เขียนด้วย `fetch()` ตรงๆ ซึ่งขัดกับกฎที่ยืนยันแล้ว (Wave 16, CRM Note Parser) ว่า n8n Code node sandbox ไม่ expose `fetch()` ให้เชื่อถือได้ — ถ้าใส่ service_role key แล้ว import โดยไม่แก้จุดนี้ด้วย จะกลับไปเจอ silent-fail แบบเดิม (fail-open ออกแบบไว้ไม่ให้บล็อก publish แต่ก็ไม่มี error โผล่ให้เห็นที่ไหนเลย)
**แก้**: เปลี่ยน `fetch()` → `this.helpers.httpRequest()` + ใส่ service_role key จริง (ดึงจาก `memory/secrets/n8n.env` ที่ใช้อยู่แล้วกับ workflow อื่น) ก่อนส่งให้ Archi import
**Verify**: หลัง Archi import + activate — เช็ค 2 ทางตรงกัน: หน้า Quality Gate tab โชว์ "1" ตรงกับ query ตรง Supabase `quality_gate_log` (1 แถว, `unverifiable_factual_claim`, blocked, human_feedback="correct")
**กฎใหม่**: เจอ dashboard 2 หน้าที่ "ดูเหมือนขัดแย้งกัน" ห้ามสรุปว่าเป็นบั๊กการแสดงผลทันที — ต้องเช็คก่อนว่าแต่ละหน้าอ่านจากแหล่งข้อมูล (state store) เดียวกันจริงไหม ถ้าคนละแหล่ง ต้องเช็คว่า pipeline ที่ควรเติมข้อมูลให้แหล่งที่สอง (feedback loop, logging node ฯลฯ) ได้ถูก deploy/activate จริงหรือยัง ก่อนแก้อะไรที่หน้า UI
**ดู**: `docs/decisions.md` ADR-021, `docs/issues-log.md`
...
### ✅ session 30 (Jul 23) — ปิดครบทุกจุดแล้ว
- [x] **Push `components/DashboardOS.tsx`** — Archi push แล้ว, commit `85e964d` ("Fix OS Dashboard Hub-bypass bug (ISSUE-017/ADR-024) + update session docs")
- [x] Verify deploy ผ่าน Vercel `list_deployments` — `dpl_EvzvQdvpDATq14jWsrHKajLeipGJ` state `READY`, target `production`, `githubCommitSha` ตรงกับ `85e964ded5dc08587f14e792e74b18052645fe22` เป๊ะ
- [x] อัปเดตกราฟ `memory/finnhouses-graph-view-v2.html` (v6) — ลบเส้นประแดง "insecure" ออกแล้ว เปลี่ยนเป็น `OS Dashboard → /api/blog/* → Hub v1` ตรงกับโค้ดที่ deploy จริง, verify ไม่มี broken link/duplicate edge เหลือ
...
### WF1 AI Quality Gate — Feedback Loop (ADR-021, session 29 ต่อๆๆๆๆ, Jul 23) — ✅ LIVE + VERIFIED (session 31, Jul 25)
...
- [x] **พบ + แก้เพิ่มก่อนส่งให้ Archi import (session 30 ต่อ)**: node "Log Quality Gate Decision" เดิมเขียนด้วย `fetch()` ตรงๆ — ขัดกับกฎที่ยืนยันแล้วจาก CRM Note Parser (Wave 16: "ห้าม fetch() ใน n8n Code node → ใช้ this.helpers.httpRequest เสมอ") เปลี่ยนเป็น `this.helpers.httpRequest()` ก่อนส่งให้ import ป้องกัน silent-fail ซ้ำรอยเดิม (Known Bug #7 class)
- [x] **Import + activate โดย Archi (Jul 25)** → verify ผ่านจริง 2 ทาง: (1) หน้า Quality Gate tab โชว์ "ตัดสินใจทั้งหมด 1" ไม่ใช่ 0 อีกต่อไป (2) เช็คตรง Supabase `quality_gate_log` มี 1 แถวจริง ตรงกับ dashboard เป๊ะ (`unverifiable_factual_claim`, blocked, human_feedback="correct") — **ปิดงานครบ ADR-021 live in production**
- [ ] Monitor ต่อเนื่อง — สะสม ≥10 ครั้งยืนยันก่อนจะเห็น % ความแม่นยำจริงในหน้า Quality Gate (ตอนนี้ 1/10)
...
- [x] เพิ่มคอลัมน์ `market_insights.human_feedback`/`human_feedback_at`
- [x] `app/api/market-intel/feedback/route.ts` (PATCH ใหม่ — ไม่มี LINE bot คู่กันเหมือน QC ฉะนั้น dashboard เป็นช่องทางยืนยันเดียว) + `app/api/market-intel/calibration/route.ts` (GET ใหม่, honest-threshold=10 เหมือนทุกจุดก่อนหน้า — เพิ่ม breakdown `by_confidence` ที่เป็นตัวชี้วัด calibration จริงๆ)
- [x] `components/MarketIntel.tsx` (หน้า `/market-intel` — คนละไฟล์กับ `MarketIntelTab` ใน DashboardOS) เพิ่ม tab ใหม่ "🎯 Calibration" พร้อมปุ่ม ✅/❌ ต่อ insight
- [x] Verify: `npx tsc --noEmit` ผ่านสะอาด, `get_advisors` ไม่มี WARN/ERROR ใหม่
...
Last updated: 2026-07-25 (session 31 — 2 เรื่องด่วนที่ Archi แจ้งเข้ามาระหว่างวัน: (1) Blog Runner บล็อกซ้ำ 2 วันติดที่ category 34 (Fix & Flip) ด้วย `brand_guardrail_violation` — root cause: prompt ของ AI Quality Gate (ADR-016) มี few-shot examples แยกแยะ "ตรวจสอบ/ที่ปรึกษา" (ผ่าน) vs "รับสร้างบ้าน" (บล็อก) แต่ไม่เคยมี example ให้ Fix & Flip renovation ของทรัพย์ที่บริษัทเป็นเจ้าของเอง (หน่วยธุรกิจหลัก 60%) โมเดลเลยเข้าใจผิดว่าเป็นการ "รับสร้างบ้าน" (Unit 1 ที่เลิกทำแล้ว) แก้โดยเพิ่ม pass-example เฉพาะ Fix & Flip เข้า prompt ในทั้งไฟล์ `(10_ai_quality_gate)` และ `(11_gate_log)` (2) Archi สังเกตว่าหน้า Overview กับ Quality Gate tab แสดงผลไม่ตรงกัน (Overview เห็น error จริง, Quality Gate โชว์ 0 ตลอด) — ตรวจแล้วไม่ใช่บั๊ก UI แต่เพราะ workflow `(11_gate_log)` ที่ต้อง log ทุกการตัดสินใจของ Gate ลง Supabase (ADR-021, ค้างจาก session 29) ยังไม่เคยถูก import — ระหว่างเตรียมไฟล์ให้ Archi import พบบั๊กซ้อนอีกจุด: node "Log Quality Gate Decision" ใช้ `fetch()` ซึ่งขัดกฎเดิม (Wave 16) ว่า n8n Code node sandbox ไม่รองรับ `fetch()` แน่นอน แก้เป็น `this.helpers.httpRequest()` + ใส่ service_role key จริง (ดึงจาก `memory/secrets/n8n.env`) ก่อนส่งให้ Archi import — Archi import + activate แล้ว, verify ผ่านจริง 2 ทาง (dashboard "1" ตรงกับ Supabase `quality_gate_log` 1 แถวจริง) — **ADR-021 ปิดครบ live in production** ดู Known Bugs #15, `docs/decisions.md` ADR-021)

Last updated (ก่อนหน้า): 2026-07-23 (session 30, ปิด session — ADR-024/ISSUE-017 **DONE ครบวงจร**: ทำ data-flow graph (Obsidian Graph view style, `memory/finnhouses-graph-view-v2.html`) verify ตรงกับโค้ดจริงหลายรอบตามที่ Archi ขอตรวจซ้ำ ("ตรงกันไหม", "อยาก" ให้ตรวจต่อ, "ประเมินก่อน") — เจอ+แก้เอง 5 จุดผิดในกราฟ (เส้นสมมติ `market_listings→CRM`, เส้นขาด `Wake-up→FB Backend`/`/api/fb/publish`/`/api/blog/*`, duplicate edge) ระหว่างตรวจเจอบั๊กจริงในโค้ด (ไม่ใช่แค่ diagram): `DashboardOS.tsx`'s `useLiveData()` ยิง Hub v1 ตรงจาก browser ไม่มี `x-hub-token` โดน 401 ทุกครั้ง (ไม่ใช่ data leak เพราะ Hub มี auth guard จริง แต่เป็น functional bug — dashboard โชว์ MOCK ค้างตลอดกาลมาตั้งแต่แรก) แก้แล้วให้เรียก `/api/blog/state` แทน — Archi push แล้ว (commit `85e964d`), verify deploy ผ่าน Vercel `list_deployments` ตรง commit เป๊ะ (`dpl_EvzvQdvpDATq14jWsrHKajLeipGJ`, `READY`), และอัปเดตกราฟ (v6) ลบเส้นประแดง "insecure" ออกแล้วให้ตรงกับ path จริงหลัง deploy — ไม่มีจุดค้างเหลือจาก session นี้ — ดู Known Bugs #14, `docs/decisions.md` ADR-024, `docs/issues-log.md` ISSUE-017)

Last updated (ก่อนหน้า): 2026-07-23 (session 29 ต่อๆๆๆๆๆๆ — ADR-023: item #3 สุดท้ายของแผน 3 ข้อ "ระบบพิสูจน์ตัวเองว่าทำงานถูก" — Market Intel confidence calibration: คอลัมน์ใหม่ `market_insights.human_feedback`/`human_feedback_at`, `app/api/market-intel/feedback/route.ts` (PATCH ใหม่) + `app/api/market-intel/calibration/route.ts` (GET ใหม่, honest-threshold=10, breakdown `by_confidence` เพื่อเช็คว่า AI confidence 1-5 ดาวแม่นจริงไหม), tab ใหม่ "🎯 Calibration" ใน `MarketIntel.tsx` (หน้า `/market-intel`) พร้อมปุ่ม ✅/❌ — verify tsc สะอาด + get_advisors ไม่มี WARN/ERROR ใหม่ — market_insights มี 135 แถวจริงอยู่แล้วแต่ human_feedback เป็น NULL ทุกแถว (ปุ่มเพิ่งมีวันนี้) รอ Archi กดยืนยันสะสม ≥10 ครั้ง — **แผน 3 ข้อ "ระบบพิสูจน์ตัวเองว่าทำงานถูก" ที่ Archi อนุมัติ "ทำทั้ง 3 อันเลย เรียงตามลำดับ" เสร็จครบทั้ง 3 ข้อแล้ว**: #1 WF1 Quality Gate feedback loop (ADR-021, รอ Archi import n8n (11_gate_log)) · #2 Land Analyzer → Deals link (ADR-022, ใช้งานได้ทันที) · #3 Market Intel calibration (ADR-023, ใช้งานได้ทันที))

Last updated (ก่อนหน้า): 2026-07-23 (session 29 ต่อๆๆๆๆๆ — ADR-022: item #3 ของแผน 3 ข้อ "ระบบพิสูจน์ตัวเองว่าทำงานถูก" — เชื่อม Land Analyzer เข้ากับ Fix & Flip Deals: คอลัมน์ใหม่ `reno_deals.land_project_id` (FK → projects, optional), ปุ่ม "🔗 สร้างดีล Fix & Flip" ใน LandAnalyzer.tsx (สร้างดีลพร้อมคำนวณ reno_budget จาก plots×area×build_cost อัตโนมัติ), `/api/projects` GET เพิ่ม `?ids=` filter, Deals.tsx โชว์ badge ROI ประเมิน+variance ต่อการ์ด + คอลัมน์ที่ 3 ในการ์ดสรุปพอร์ต — verify tsc สะอาด + get_advisors ไม่มี WARN/ERROR ใหม่ — **รอ Archi**: เริ่มใช้ปุ่มจริงจาก Land Analyzer แล้วปิดดีลอย่างน้อย 1 ดีลเพื่อดูว่า ROI variance ออกค่าที่สมเหตุสมผลไหม — item #2 (WF1 Quality Gate feedback loop, ADR-021) เสร็จก่อนหน้านี้แล้ว รอ Archi import (11_gate_log) — เหลือ item #3 สุดท้ายของแผน: Market Intel confidence calibration)

Last updated (ก่อนหน้า): 2026-07-23 (session 29 ต่อๆๆๆๆ — ADR-021: item #2 ของแผน 3 ข้อ "ระบบพิสูจน์ตัวเองว่าทำงานถูก" ที่ Archi อนุมัติ "ทำทั้ง 3 อันเลย เรียงตามลำดับ" — เพิ่ม feedback loop ให้ WF1 AI Quality Gate (ADR-016): ตาราง Supabase `quality_gate_log` ใหม่ (RLS 0-policy, service_role only), n8n node ใหม่ "Log Quality Gate Decision" คั่น `Parse AI Gate Result` → `Merge Guards` (fail-open, ไม่บล็อก pipeline หลัก) ในไฟล์ใหม่ `(11_gate_log).json` (ไฟล์เดิม (10) ไม่ถูกแตะ) — validate ครบ: parse JSON, node count 22→23, wiring 2 จุด, **ทุก node reachable จาก Webhook** (เช็คเพิ่มจาก ADR-016 ที่ไม่ได้ทำจุดนี้) — ฝั่ง Next.js: `app/api/quality-gate/accuracy/route.ts` (GET) + `app/api/quality-gate/feedback/route.ts` (PATCH ใหม่ — Gate ไม่มี LINE bot คู่กันเหมือน QC ฉะนั้น dashboard นี้เป็นช่องทางยืนยันเดียว) + `components/QualityGateAccuracy.tsx` (ปุ่ม ✅/❌ inline) + tab ใหม่ "🛡️ Quality Gate" ใน DashboardOS — เพิ่ม metric ใหม่ที่ QC ไม่มี: false_positives/false_negatives (Gate ผิดพลาดได้ 2 ทิศทาง) — verify tsc สะอาด — **รอ Archi**: ใส่ service_role key จริง + import + activate (11_gate_log) แทน (10) แล้วเริ่มกดยืนยันสะสม ≥10 ครั้งในหน้า Quality Gate — ต่อไปคือ item #3/#3 ของแผน: Land Analyzer → Deals link, แล้ว Market Intel confidence calibration ตามลำดับที่ Archi ขอ)
